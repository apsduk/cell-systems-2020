...
for i_1 = 1:length(allowableomega) % omegagP
    for i_2 = 1:length(v_betaf) % betafP
        for i_3 = 1:length(v_betaf) % betafF
            for i_4 = 1:length(v_proteins) % protein characteristics
                ...
                X00(fid, 7) = v_betaf(i_3);     % betafF
                X00(fid, 9) = v_xif_const_2; % xifF
                X00(fid,10) = v_xif_const_1;  % xifP
                X00(fid,11) = v_xif(proteinidx(i_4,2));  % xifR
                X00(fid,14) = v_mu(proteinidx(i_4,2));   % alpharF
                X00(fid,15) = v_eta(proteinidx(i_4,2));  % etaF
...
parfor i = 1:length(X0(:,1))
    ...
    % ODEs
    f_1 = @(T, Y) op_or(T, Y, [X(2:9), X(2:9)], X(11:18), X(20:25), X(26:31), [X(10), X(19),    0,    0, X(32)], [], X(33:43));
    f_2 = @(T, Y) op_or(T, Y, [X(2:9), X(2:9)], X(11:18), X(20:25), X(26:31), [X(10), X(19), X(1),    0, X(32)], [], X(33:43));
    f_3 = @(T, Y) op_or(T, Y, [X(2:9), X(2:9)], X(11:18), X(20:25), X(26:31), [X(10), X(19), X(1), X(1), X(32)], [], X(33:43));
