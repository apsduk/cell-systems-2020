function dYdt = sqtr_or(T, Y, circPR, ornapPR, oriboPR, hostPR, plasmidPR, rnapcontPR, ribocontPR)
...
% o-rnap controller
xifQ = rnapcontPR(1); xirQ = rnapcontPR(2); tauQ = rnapcontPR(3); deltarQ = rnapcontPR(4); deltadQ = rnapcontPR(5); alphafQ = rnapcontPR(6); alpharQ = rnapcontPR(7);
...
tauF = phiR*tauF;
% tauQ = tauQ; % as this is not protein coding

%% %%%%% APPLY CONSERVATION LAWS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
gP = omegagP/lambda - xP;
...
hP = Ptot - xF - xR;
hR = Rtot - oR - cP - c1 - c2 - cF;

%% %%%%% ODEs %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% o-rnap
dxP = xifP*gP*oP - (xirP + tauP)*xP - lambda*xP;
dmP = tauP*xP - betafP*mP*hR + (betarP + gammaP)*cP ...
    - alphafQ*mP*rQ + alpharQ*dQ ...
    - (deltamP + lambda)*mP;
...
drQ = tauQ*xQ ...
    - alphafQ*mP*rQ + alpharQ*dQ ...
    - (deltarQ + lambda)*rQ;
ddQ = alphafQ*mP*rQ - alpharQ*dQ - (deltadQ + lambda)*dQ;

%% %%%%% RETURN dYdt %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dYdt = [dxP; dmP; dcP; doP; dxR; drR; doR; ...
    dx1; dm1; dc1; dp1; dx2; dm2; dc2; dp2; ...
    dkR; dxF; dmF; dcF; dpF; dxQ; drQ; ddQ];
