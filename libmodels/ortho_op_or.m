function dY = ortho_op_or(T, Y, circPR, ornapPR, oriboPR, hostPR, plasmidPR, ~, ribocontPR)
...
omegagF = phiP*omegagF;
tau1 = phiR*tau1; tau2 = phiR*tau2;
tauP = phiR*tauP; tauF = phiR*tauF;
...
dmP = tauP*xP - betafP*mP*oR + (betarP + gammaP)*cP - (deltamP + lambda)*mP;
dcP = betafP*mP*oR - (betarP + gammaP)*cP - lambda*cP;
doP = oleak + gammaP*cP - (deltaoP + lambda)*oP ...
    - xifP*gP*oP + (xirP + tauP)*xP ...
    - xifR*gR*oP + (xirR + tauR)*xR ...
    - xif1*g1*oP + (xir1 + tau1)*x1 ...
    - xif2*g2*oP + (xir2 + tau2)*x2 ...
    - xifF*gF*oP + (xirF + tauF)*xF;

% orthogonal ribosomes
dxR = xifR*gR*oP - (xirR + tauR)*xR - lambda*xR;
drR = tauR*xR - (deltarR + lambda)*rR - rhof*rR*hR + rhor*oR;
doR = rhof*rR*hR - rhor*oR - lambda*oR ...
    - betafP*mP*oR + (betarP + gammaP)*cP ...
...
dxF = xifF*gF*oP - (xirF + tauF)*xF - lambda*xF;
...
%% %%%%% RETURN dYdt %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dY = [dxP; dmP; dcP; doP; dxR; drR; doR; ...
