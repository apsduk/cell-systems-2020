...
for i_1 = 1:length(allowableomega) % omegagP
    for i_2 = 1:length(v_betaf) % betafP
        for i_3 = 1:length(v_betaf) % betafF
            for i_4 = 1:length(v_xif_const) % xifP
                for i_5 = 1:length(v_xif_const) % xifF
                    for i_6 = 1:length(v_proteins) % protein characteristics
                        
                        fid = fid + 1;
                        X00(fid, 1) = allowableomega(i_1, 1);       % omegagP
                        X00(fid, 2) = allowableomega(i_1, 2);       % omegagR
                        X00(fid, 3) = allowableomega(i_1, 3);       % omegagF
                        X00(fid, 4) = v_betaf(i_2);                 % betafP
                        X00(fid, 5) = v_betaf(i_3);                 % betafF
                        X00(fid, 6) = v_xif_const(i_4);             % xifP
                        X00(fid, 7) = v_xif_const(i_5);             % xifF
                        X00(fid, 8) = v_xif(proteinidx(i_6,1));     % xifR
                        X00(fid, 9) = v_mu(proteinidx(i_6,1));      % alpharF
                        X00(fid,10) = v_eta(proteinidx(i_6,1));     % etaF
...
parfor i = 1:length(X0(:,1))
    ...
    % ODES
    f_1 = @(T, Y) ortho_op_or(T, Y, [X(2:9), X(2:9)], X(11:18), X(20:25), X(26:31), [X(10), X(19),     0,     0, X(32)], [], X(33:43));
    f_2 = @(T, Y) ortho_op_or(T, Y, [X(2:9), X(2:9)], X(11:18), X(20:25), X(26:31), [X(10), X(19), X( 1),     0, X(32)], [], X(33:43));
    f_3 = @(T, Y) ortho_op_or(T, Y, [X(2:9), X(2:9)], X(11:18), X(20:25), X(26:31), [X(10), X(19), X( 1), X( 1), X(32)], [], X(33:43));
